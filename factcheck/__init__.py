...
class FactCheck:
    ...
    def check_text(self, raw_text: str):
        ...
        # Limit to max 3 claims to reduce token usage
        MAX_CLAIMS = 3
        if len(claims) > MAX_CLAIMS:
            logger.info(f"== Limiting claims from {len(claims)} to {MAX_CLAIMS}")
    ...
    def _merge_claim_details(
        ...
        for i, (claim, origin) in enumerate(claim2doc.items()):
            if claim in claim2verifications:
                ...
                trusted_sources = ["bbc.com", "reuters.com", "apnews.com", "thehindu.com", "ndtv.com"]
                blog_sources = ["blogspot.com", "wordpress.com", "medium.com", "tumblr.com", "substack.com", "quora.com", "reddit.com", "twitter.com", "x.com"]

                for ev in evidences:
                    rel = str(ev.relationship).strip().upper()
                    url = str(ev.url).lower()
                    
                    weight = 1.0
                    if any(domain in url for domain in trusted_sources):
                        weight = 2.0
                    elif any(domain in url for domain in blog_sources):
                        weight = 0.5
                    ...
                    if rel == "SUPPORTS":
                        ...
                        refutes += weight
                    elif rel == "CONTROVERSIAL":
                        contro += weight
                
                if supports + refutes + contro == 0:
                    factuality = "No evidence found."
                else:
                    # Treat controversial as 0.5 score
                    factuality = (supports + 0.5 * contro) / (supports + refutes + contro)
